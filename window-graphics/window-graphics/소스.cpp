...
// 기존의 face1Colors ~ face7Colors 선언 부분을 다음과 같이 변경
...
// 각 큐브의 색상을 정의
std::array<std::array<glm::vec3, 4>, 6> face1Colors = createFaceColors(glm::vec3(1.0f, 0.0f, 0.0f));    // 빨간색
std::array<std::array<glm::vec3, 4>, 6> face2Colors = createFaceColors(glm::vec3(1.0f, 1.0f, 0.0f));    // 노란색
std::array<std::array<glm::vec3, 4>, 6> face3Colors = createFaceColors(glm::vec3(1.0f, 0.7f, 1.0f));    // 분홍색
std::array<std::array<glm::vec3, 4>, 6> face4Colors = createFaceColors(glm::vec3(1.0f, 0.5f, 1.0f));    // 연한 분홍색
std::array<std::array<glm::vec3, 4>, 6> face5Colors = createFaceColors(glm::vec3(0.0f, 0.5f, 0.8f));    // 하늘색
std::array<std::array<glm::vec3, 4>, 6> face6Colors = createFaceColors(glm::vec3(0.2f, 0.0f, 0.8f));    // 보라색
std::array<std::array<glm::vec3, 4>, 6> face7Colors = createFaceColors(glm::vec3(1.0f, 1.0f, 1.0f));    // 하얀색
...
void drawScene() {
    ...
    glBindVertexArray(vao[2]);
    model = glm::mat4(1.0f);
    model = glm::rotate(model, glm::radians(planetRotation1), glm::vec3(0.0f, 1.0f, 0.0f));  // y축 기준 회전
    model = glm::translate(model, glm::vec3(orbit1Radius, 0.9f, 0.0f));  // x축 방향으로 이동한 후 회전
    ...
    glBindVertexArray(vao[3]);
    model = glm::mat4(1.0f);
    model = glm::rotate(model, glm::radians(planetRotation2), glm::vec3(0.0f, 1.0f, 0.0f));
    model = glm::translate(model, glm::vec3(orbit2Radius, 0.5f, 0.0f));
    ...
    glBindVertexArray(vao[4]);
    model = glm::mat4(1.0f);
    model = glm::rotate(model, glm::radians(planetRotation3), glm::vec3(0.0f, 1.0f, 0.0f));
    model = glm::translate(model, glm::vec3(orbit3Radius, 0.2f, 0.0f));
...
int main(int argc, char** argv) {
    ...
    read_obj_file("pyramid.obj", cube2VertexData, cube2Indices, face2Colors);
    read_obj_file("sphere.obj", cube3VertexData, cube3Indices, face3Colors);
    read_obj_file("sphere.obj", cube4VertexData, cube4Indices, face4Colors);
    read_obj_file("sphere.obj", cube5VertexData, cube5Indices, face5Colors);
